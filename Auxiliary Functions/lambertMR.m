...
RIM2   = dot(RI,RI); % Magnitude squared
RIM    = sqrt(RIM2); % Magnitude
RFM2   = dot(RF,RF);
RFM    = sqrt(RFM2); 
CTH    = dot(RI,RF)/(RIM*RFM); % Cos of the angle between initial and final radius
CR     = cross(RI,RF);
STH    = norm(CR)/(RIM*RFM); % Sin of the angle between initial and final radius
...
S= (RIM + RFM + C)/2; % Semi-perimeter of the triangle
BETA   = 2*asin(sqrt((S-C)/S));
PMIN   = TWOPI*sqrt(S^3/(8*MU)); % Period of the ellipse
TMIN   = PMIN*(pi-BETA+sin(BETA))/(TWOPI);
LAMBDA = B1*sqrt((S-C)/S); % q on the NASA paper
